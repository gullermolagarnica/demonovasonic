...
class BedrockStreamManager:
    ...
    def _escape_json_string(value: str) -> str:
        """Escape a string so it can be safely embedded in JSON templates."""
        if value is None:
            return ""
    ...
    async def initialize_stream(self):
        ...
        try:
            ...
            12. CONTEXTO EMOCIONAL Y URGENCIA  
            Los usuarios pueden llegar con sentido de urgencia relacionado con su salud.  
            Por ello:  
            - Prioriza respuestas sintetizadas, precisas y tranquilizadoras.  
            - Transmite calma y apoyo en todo momento.  
            - Ajusta el tono según la gravedad percibida del caso, mostrando empatía y acompañamiento.  
            - Refuerza siempre que estás para asistir y orientar, incluso si la situación es tensa o delicada.

            ---

            """).strip()
            escaped_system_prompt = self._escape_json_string(default_system_prompt)
            
            # Send initialization events
            prompt_event = self.start_prompt()
            text_content_start = self.TEXT_CONTENT_START_EVENT % (self.prompt_name, self.content_name, "SYSTEM")
            text_content = self.TEXT_INPUT_EVENT % (self.prompt_name, self.content_name, escaped_system_prompt)
