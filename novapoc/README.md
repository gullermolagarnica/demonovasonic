...
4. **nova_sonic_web.py**: A FastAPI-based web experience that streams microphone audio from your browser to the Nova Sonic model in real time. It reuses the full console pipeline but moves capture/playback to the browser, giving you a lightweight UI for speaking with the model without installing audio drivers locally.
...
- Browser-based interface with live audio capture/playback (nova_sonic_web.py)
...
The `nova_sonic_web.py` application exposes the same streaming flow through a very simple web UI. Run it with your AWS credentials configured and then open your browser to start talking to Nova Sonic:

```bash
python -m novapoc.nova_sonic_web
```

By default the server auto-generates a self-signed certificate and serves over HTTPS so that browsers allow microphone access. Open [https://localhost:8000](https://localhost:8000) (accept the browser warning once), grant the page microphone access, click **Iniciar conversación**, and begin speaking. Press **Detener** in the UI to let the model respond, or refresh/close the tab to end the session cleanly.

You can change the network binding:

```bash
# Bind to a specific interface/IP and store certs under ./certs
python -m novapoc.nova_sonic_web --host 192.168.0.10 --port 8443 --cert-dir ./certs

# Run without HTTPS (only works if you access via http://localhost)
python -m novapoc.nova_sonic_web --insecure
```
